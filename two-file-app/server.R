...
server <- function(input, output) {
  ...
  trout_filtered_df <- reactive({
    ...
    # create a message to guide users
    validate(
      need(length(input$channel_type_input) > 0, "Please select at least one channel type"),
      need(length(input$section_input) > 0, "Please select at least one section type")
    )
  ...
  # add alt-text ------
  alt = "Trout plot alt text"
  
  ) 
  
  # filter penguin data for island
  island_df <- reactive ({
